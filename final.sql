...
-- =====================================================================
-- 5. CAMADA DE SEGURANÇA - USUÁRIOS E VIEWS DE ACESSO
-- =====================================================================
...
-- ----- Criação de Logins (Users) e permissões -----
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'rh')          THEN 
        CREATE USER rh WITH PASSWORD '456';          
    END IF;
    
    IF NOT EXISTS (SELECT FROM pg_catalog.pg_roles WHERE rolname = 'diretor')     THEN 
        CREATE USER diretor WITH PASSWORD '789';     
    END IF;
END $$;
